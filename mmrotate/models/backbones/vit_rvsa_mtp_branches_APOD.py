...
from ..duomi_adapter import Adapter_single, Adapter_dual
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.petl = petl
        if petl:
            self.duomi_adapter1 = Adapter_single(dim=dim, adapter_dim=64, sort='MSWFA')
            self.duomi_adapter2 = Adapter_single(dim=dim, adapter_dim=64, sort='MSWFA')
    ...
    def forward(self, x, H, W):
        ...
        if self.petl:
            if self.gamma_1 is None:
                ...
                x = self.duomi_adapter1(x, H, W)
                x = x + self.drop_path(self.mlp(self.norm2(x)))
                x = self.duomi_adapter2(x, H, W)
            else:
...
class RVSA_MTP_branches_APOD(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 petl=False, only_head=False):
        if petl:
            self.duomi_adapter3 = Adapter_dual(x_dim=embed_dim, y_dim=embed_dim, adapter_dim=64, sort='SFRA')
            self.duomi_adapter4 = Adapter_dual(x_dim=embed_dim, y_dim=embed_dim, adapter_dim=64, sort='SFRA')
            self.duomi_adapter5 = Adapter_dual(x_dim=embed_dim, y_dim=embed_dim, adapter_dim=64, sort='SFRA')
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.in_chans = in_chans
        self.l1loss = nn.L1Loss()
        self.duomi_adapter_fog = Adapter_single(dim=embed_dim, adapter_dim=64, sort='FARA')
    ...
    def forward_features(self, x):
        img_fog, img_clean = x[0], x[1]

        B, C, H, W = img_fog.shape

        img_fog, (Hp, Wp) = self.patch_embed(img_fog)
        img_clean, _ = self.patch_embed(img_clean)

        batch_size, seq_len, _ = img_fog.size()

        if self.pos_embed is not None:
            img_fog = img_fog + self.pos_embed
        img_fog = self.pos_drop(img_fog)
        img_fog = self.duomi_adapter_fog(img_fog, Hp, Wp)
        if self.pos_embed is not None:
            img_clean = img_clean + self.pos_embed
        img_clean = self.pos_drop(img_clean)


        loss_feat = []
        loss_feat_sub = self.l1loss(img_fog,img_clean)
        loss_feat.append(loss_feat_sub)
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                img_fog = checkpoint.checkpoint(blk, img_fog, Hp, Wp)
            else:
                img_fog = blk(img_fog, Hp, Wp)

            # if self.use_checkpoint:
            #     img_clean = checkpoint.checkpoint(blk, img_clean, Hp, Wp, 'clean')
            # else:
            #     img_clean = blk(img_clean, Hp, Wp, 'clean')

            # loss_feat_sub = self.l1loss(img_fog, img_clean)
            # loss_feat.append(loss_feat_sub)
                
            if i in self.out_indices:
                features.append(img_fog)

        features[2] = self.duomi_adapter3(features[2], features[3], Hp, Wp)
        features[1] = self.duomi_adapter4(features[1], features[2], Hp, Wp)
        features[0] = self.duomi_adapter5(features[0], features[1], Hp, Wp)
        ...
        return tuple(features), loss_feat
    ...
    def forward_test(self, x):
        ...
        if self.pos_embed is not None:
            x = x + self.pos_embed
        x = self.pos_drop(x)
        x = self.duomi_adapter_fog(x, Hp, Wp)
